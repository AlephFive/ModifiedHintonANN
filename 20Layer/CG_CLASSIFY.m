...
% Do decomversion.
 ...
 w_class = reshape(VV(xxx+1:xxx+(l21+1)*l22),l21+1,l22);
%notice
  ...
  targetout = exp(w20probs*w_class);
  targetout = targetout./repmat(sum(targetout,2),1,10);
  f = -sum(sum( target(:,1:end).*log(targetout))) ;

IO = (targetout-target(:,1:end));
Ix_class=IO; 
dw_class =  w20probs'*Ix_class; 

Ix20 = (Ix_class*w_class').*w20probs.*(1-w20probs);
Ix20 = Ix20(:,1:end-1);
dw20 =  w20probs'*Ix20;
...
dw12(:)' dw13(:)' dw14(:)' dw15(:)' dw16(:)' dw17(:)' dw18(:)' dw19(:)' dw20(:)'
dw_class(:)']'; 
