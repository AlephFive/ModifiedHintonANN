...
w_class = 0.1*randn(size(w8,2)+1,10);
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    VV = [w_class(:)']';
    Dim = [l9; l10];
    [X, fX] = minimize(VV,'CG_CLASSIFY_INIT',max_iter,Dim,w8probs,targets);
    w_class = reshape(X,l9+1,l10);

  else
    VV = [w1(:)' w2(:)' w3(:)' w4(:)' w5(:)' w6(:)' w7(:)' w8(:)' w_class(:)']';
    ...
    w_class = reshape(X(xxx+1:xxx+(l9+1)*l10),l9+1,l10);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 save mnistclassify_weights w1 w2 w3 w4 w5 w6 w7 w8 w_class
