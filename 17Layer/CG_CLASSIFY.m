...
% Do decomversion.
 ...
 w_class = reshape(VV(xxx+1:xxx+(l18+1)*l19),l18+1,l19);
%notice
  ...
  targetout = exp(w17probs*w_class);
  targetout = targetout./repmat(sum(targetout,2),1,10);
  f = -sum(sum( target(:,1:end).*log(targetout))) ;

IO = (targetout-target(:,1:end));
Ix_class=IO; 
dw_class =  w17probs'*Ix_class; 

Ix17 = (Ix_class*w_class').*w17probs.*(1-w17probs);
Ix17 = Ix17(:,1:end-1);
dw17 =  w17probs'*Ix17;
