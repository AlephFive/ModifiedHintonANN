...
w_class = 0.1*randn(size(w6,2)+1,10);
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    VV = [w_class(:)']';
    Dim = [l7; l8];
    [X, fX] = minimize(VV,'CG_CLASSIFY_INIT',max_iter,Dim,w6probs,targets);
    w_class = reshape(X,l7+1,l8);

  else
    VV = [w1(:)' w2(:)' w3(:)' w4(:)' w5(:)' w6(:)' w_class(:)']';
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 save mnistclassify_weights w1 w2 w3 w4 w5 w6 w_class
