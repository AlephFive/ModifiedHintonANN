...
% Do decomversion.
 ...
 w_class = reshape(VV(xxx+1:xxx+(l19+1)*l20),l19+1,l20);
%notice
  ...
  targetout = exp(w18probs*w_class);
  targetout = targetout./repmat(sum(targetout,2),1,10);
  f = -sum(sum( target(:,1:end).*log(targetout))) ;

IO = (targetout-target(:,1:end));
Ix_class=IO; 
dw_class =  w18probs'*Ix_class; 

Ix18 = (Ix_class*w_class').*w18probs.*(1-w18probs);
Ix18 = Ix18(:,1:end-1);
dw18 =  w18probs'*Ix18;
...
dw12(:)' dw13(:)' dw14(:)' dw15(:)' dw16(:)' dw17(:)' dw18(:)'
dw_class(:)']'; 
