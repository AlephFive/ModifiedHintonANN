...
% Do decomversion.
 ...
 w_class = reshape(VV(xxx+1:xxx+(l20+1)*l21),l20+1,l21);
%notice
  ...
  targetout = exp(w19probs*w_class);
  targetout = targetout./repmat(sum(targetout,2),1,10);
  f = -sum(sum( target(:,1:end).*log(targetout))) ;

IO = (targetout-target(:,1:end));
Ix_class=IO; 
dw_class =  w19probs'*Ix_class; 

Ix19 = (Ix_class*w_class').*w19probs.*(1-w19probs);
Ix19 = Ix19(:,1:end-1);
dw19 =  w19probs'*Ix19;
...
dw12(:)' dw13(:)' dw14(:)' dw15(:)' dw16(:)' dw17(:)' dw18(:)' dw19(:)'
dw_class(:)']'; 
