...
w_class = 0.1*randn(size(w7,2)+1,10);
...
%%%%%%%%%%%%%%% PERFORM CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  if epoch<6  % First update top-level weights holding other weights fixed. 
    ...
    VV = [w_class(:)']';
    Dim = [l8; l9];
    [X, fX] = minimize(VV,'CG_CLASSIFY_INIT',max_iter,Dim,w7probs,targets);
    w_class = reshape(X,l8+1,l9);

  else
    VV = [w1(:)' w2(:)' w3(:)' w4(:)' w5(:)' w6(:)' w7(:)' w_class(:)']';
    ...
    w_class = reshape(X(xxx+1:xxx+(l8+1)*l9),l8+1,l9);
...
%%%%%%%%%%%%%%% END OF CONJUGATE GRADIENT WITH 3 LINESEARCHES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 save mnistclassify_weights w1 w2 w3 w4 w5 w6 w7 w_class
